...
void metal_set_render_target(RT_ResourceHandle texture)
{
	// Flush rasterizer
	RT_RasterRender();
...
void metal_loadbmtexture_f(grs_bitmap* bm, int texfilt)
{
	...
	{
		// Upload texture
		uint32_t* pixels = metal_load_bitmap_pixel_data(&g_thread_arena, bm);
...
void metal_init_font(grs_font* font)
{
	...
	metal_font_choose_size(font, gap, &tw, &th);
	data = d_malloc(tw * th);
	...
	metal_init_texture(&font->ft_parent_bitmap);
	...
	metal_loadbmtexture_f(&font->ft_parent_bitmap, GameCfg.TexFilt);
}
...
//unsigned char metal_ugpixel(grs_bitmap* bitmap, int x, int y)
//{
	...
	while (next_row != NULL)
	{
		...
		while (*text_ptr)
		{
			...
			if (grd_curcanv->cv_font->ft_flags & FT_COLOR)
				metal_ubitmapm_cs(xx, yy, FONTSCALE_X(ft_w), FONTSCALE_Y(grd_curcanv->cv_font->ft_h), &grd_curcanv->cv_font->ft_bitmaps[letter], -1, F1_0);
			else {
				if (grd_curcanv->cv_bitmap.bm_type == BM_OGL)
					metal_ubitmapm_cs(xx, yy, ft_w * (FONTSCALE_X(grd_curcanv->cv_font->ft_w) / grd_curcanv->cv_font->ft_w), FONTSCALE_Y(grd_curcanv->cv_font->ft_h), &grd_curcanv->cv_font->ft_bitmaps[letter], grd_curcanv->cv_font_fg_color, F1_0);
				else
...
bool metal_ubitmapm_cs(int x, int y, int dw, int dh, grs_bitmap* bm, int c, int scale)
{
	...
	{
		metal_init_texture(bm);
		metal_loadbmtexture_f(bm, GameCfg.TexFilt);
	}
